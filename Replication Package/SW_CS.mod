...
var   labobs robs pinfobs dy dc dinve dw  ewma epinfma  zcapf rkf kf pkf    cf invef yf labf wf rrf mc zcap rk k pk    c inve y lab pinf w r a  b g qs  ms  spinf sw kpf kp rr v s vstar;    
...
ctrend 
conster cg cgamma clandap cbetabar cr cpie crk cw cikbar cik clk cky ciy ccy crkky cwhlc cwly R crpis gamma;
...
crpi        = 0;2.0443; 1.488; //Taylor rule reaction to inflation
...
// Resource constraint
          ...
          v = cbeta^(-1)*(1-gamma)*v(-1) + r - (pinf- constepinf) + g - s;

          s   = R*vstar(-1) + crpis*(pinf- constepinf) - eps_s;

          cbeta* vstar = vstar(-1) - s;
