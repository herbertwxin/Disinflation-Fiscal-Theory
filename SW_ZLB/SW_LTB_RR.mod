...
var   labobs robs pinfobs dy dc dinve dw  ewma epinfma  zcapf rkf kf pkf    cf invef yf labf wf rrf mc zcap rk k pk    c inve y lab pinf w r a  b g qs  ms  spinf sw kpf kp v rn q s rr; //vstar;    
...
ctrend 
conster cg cgamma clandap cbetabar cr cpie crk cw cikbar cik clk cky ciy ccy crkky cwhlc cwly R omega ;//phi_s;
...
// Resource constraint
          ...
          //s   = vstar + phi_s*(pinf- constepinf);
        
          //s   = 0.2*v(-1) + (y-yf) + 0.25*(pinf- constepinf) - eps_s - 0.5*eps_s(-1);
