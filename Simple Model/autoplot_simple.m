function [fig, sr_value_rounded] = autoplot_simple(model_name, options_,M_,oo_, CPIE, CRPI, RR, Ta, Tstar)
...
fig = figure;
subplot(2,2,1)
plot(y_non(base_model.variable.i,:),'Color', '#024b79', 'LineWidth', 2)
ylabel('Interest rate/real rate')
hold on
plot(y_non(base_model.variable.r,:),'Color', '#C2444E', 'LineStyle', '-.', 'LineWidth', 2)
...
hold on
subplot(2,2,3)
plot(y_non(base_model.variable.x,:),'Color', '#024b79', 'LineWidth', 2)
% if model_name == 'NK'
%     axis([0 T -0.1 0.1])
% end
...
plot(y_non(base_model.variable.s,:),'Color', '#C2444E', 'LineStyle', '-.', 'LineWidth', 2)
legend('Debt', 'Surplus')
% if model_name == 'NK'
%     axis([0 T -0.1 0.1])
% end
